...
_.assign(AuthenticatedClient.prototype, new function() {
  ...
  prototype._transferFunds = function(params, callback) {
    var self = this;
    _.forEach(['type', 'amount'], function(param) {
      if (params[param] === undefined) {
        throw "`opts` must include param `" + param + "`";
      }
    });
    ...
    if (params.type === 'withdraw') {
      uriParts.push('withdrawals');
    }

    if (params.coinbase_account_id) {
      uriParts.push('transfers');
    } else if (params.crypto_address) {
      uriParts.push('crypto');
    } else if (params.payment_method_id) {
      uriParts.push('payment-method');
    } else {
      throw "`opts` must include a to address";
    }

    var opts = { 'body': params };
    return prototype.post.call(self, uriParts, opts, callback);
  };
